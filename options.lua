...
-- * value: true if key is in optional flags array, or
...
--
-- flags: a list of options which take no arguments and will be set to true if
--        found, otherwise not set ie. nil
--
-- ex: local opt, argi = options.parse(arg, {"a"})
--     "--foo 123 -a -bar abc -- baz" -> {"foo"=123, "bar"="abc", "a"=true}, 7
--
-- returns options key/value hash & first non-option pos in arg array
-- non-options pos is 0 if no remaining args
function options.parse(arg, flags)
    local opt = {}
    local i = 1 -- arg index
    while i <= #arg do
        if arg[i] == "--" then
        ...
        i = i + 1
        if key then
            if flags and options.contains(flags, key) then
                -- flag
                opt[key] = true
            elseif arg[i] == nil or options.isopt(arg[i]) then
                -- assume flag
                opt[key] = true
            else
                -- value
...
function options.contains(table, value)
    for i=1,#table do
